...
class Parser:
    ...
    def parse_function_definition(self) -> ASTNode:
        ...
        name = self.expect("IDENTIFIER", expected="имя функции").value
    ...
    def parse_parameter_list(self) -> List[ASTNode]:
        ...
        while True:
            ...
            name = self.expect("IDENTIFIER", expected="имя параметра").value
            params.append(ASTNode("Param", {"type": param_type, "name": name}))
    ...
    def parse_for_stmt(self) -> ASTNode:
        ...
        if self.is_declaration_start():
            ...
            init_name = self.expect("IDENTIFIER", expected="имя переменной цикла").value
            if self.match("DELIMITER", ":"):
                ...
                return ASTNode(
                    "RangeForStmt",
                    {
                        "var": ASTNode("VarDecl", {"type": init_type, "name": init_name, "init": None}),
                        "iterable": iterable,
                        "body": body,
                    },
                )
            ...
            init = ASTNode("VarDecl", {"type": init_type, "name": init_name, "init": init_expr})
    ...
    def parse_var_decl_stmt(self, require_semicolon: bool) -> ASTNode:
        ...
        name = self.expect("IDENTIFIER", expected="имя переменной").value
        init = None
        if self.match("OPERATOR", "="):
            init = self.parse_expression()
        node = ASTNode("VarDeclStmt", {"declarations": [ASTNode("VarDecl", {"type": var_type, "name": name, "init": init})]})
